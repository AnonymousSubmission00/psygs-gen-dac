...
class SyGSTemplateTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "write and read memory" in
    test(new SyGSTemplate(2, 1, 2, 10, 10, false, 10, 10, 10, 4, 16, 8, 11, 53, 10, 10, false))
      .withAnnotations(Seq(WriteVcdAnnotation)) { c =>
        ...
        c.ctrl_io.rocc_cmd.resp_rdy.poke(true.B)

        c.ctrl_io.rocc_cmd.busy.expect(false.B)
        c.ctrl_io.rocc_cmd.cmd_rdy.expect(true.B)
  ...
  it should "work with smol matrix" in
    test(new SyGSTemplate(2, 1, 2, 10, 10, false, 10, 10, 10, 4, 16, 8, 11, 53, 10, 10, false))
      .withAnnotations(Seq(WriteVcdAnnotation)) { c =>
        ...
        var cycles = 1
        while (!c.ctrl_io.rocc_cmd.resp_valid.peekBoolean()) {
  ...
  it should "work with test matrix 3x3" in
    test(new SyGSTemplate(1, 1, 2, 8, 8, false, 2, 2, 8, 4, 17, 12, 11, 53, 8, 8, false))
      .withAnnotations(Seq(WriteVcdAnnotation)) { c =>
        ...
        val variables = ListBuffer.fill(3)(0L)
        val acc1Mem = ListBuffer[Long](3L,3L,1L,3L,1L,0L,4715937977367081240L,1L,0L,-4515190131097614053L,1L,0L,4714871012338188716L,4643439914237558784L,4635611391447793664L,4643844534516580352L,0L,1L,2L)
  ...
  it should "work with test matrix 5x5" in
    test(new SyGSTemplate(2, 1, 2, 8, 8, false, 2, 2, 8, 4, 17, 12, 11, 53, 8, 8, false))
      .withAnnotations(Seq(WriteVcdAnnotation)) { c =>
        ...
        val variables = ListBuffer.fill(5)(0L)
        val acc1Mem = ListBuffer[Long](2L,2L,1L,2L,1L,0L,-4515190131097614053L,1L,0L,4715303177709975584L,4644477853214179328L,4649993003539103744L,1L,3L)
        val acc2Mem = ListBuffer[Long](3L,3L,1L,3L,1L,0L,4715937977367081240L,1L,0L,4714871012338188716L,1L,0L,4715937977367081240L,4641522365958717440L,4647864349027729408L,4646325032748843008L,0L,2L,4L)
  ...
  def test(c: SyGSTemplate, backwards: Boolean, colors: Int, rowsPerColor: Int, numberOfNonZerosPerRow: Int): Unit = {
    ...
    for (color <- 0 until colors) {
      ...
      for (r <- 0 until rowsPerColorPerAccumulator) {
        ...
        c.ctrl_io.rocc_cmd.cmd_valid.poke(false.B)
    ...
    c.clock.step()
    c.ctrl_io.rocc_cmd.cmd_rdy.expect(true)

    start(c, backwards)
    c.clock.step()

    var cycles = 1
    while (c.ctrl_io.rocc_cmd.busy.peekBoolean()) {
    ...
    c.ctrl_io.rocc_cmd.resp_valid.expect(true.B)
    println("Computation ended!\nDuration: " + c.ctrl_io.rocc_cmd.resp_data.peek().litValue + " cycles")
  ...
  def start(c: SyGSTemplate, backwards: Boolean): Unit = {

    c.ctrl_io.rocc_cmd.cmd_inst.opcode.poke(SyGSOpCode.EXEC.id.U)
    c.ctrl_io.rocc_cmd.cmd_inst.funct.poke(if(backwards) 1.U else 0.U)
    c.ctrl_io.rocc_cmd.cmd_rs1.poke(0.U)
    c.ctrl_io.rocc_cmd.cmd_rs2.poke(0.U)
    c.ctrl_io.rocc_cmd.cmd_valid.poke(true.B)
  ...
  def writeMem(value: Double, index: Int, interface: Int, c: SyGSTemplate): Unit = {

    writeMem(doubleToUInt(value), index, interface, c)
  }

  def writeMem(value: Long, index: Int, interface: Int, c: SyGSTemplate): Unit = {

    writeMem(longToUInt(value), index, interface, c)
  }

  def writeMem(value: UInt, index: Int, interface: Int, c: SyGSTemplate): Unit = {

    c.ctrl_io.rocc_cmd.cmd_inst.opcode.poke(SyGSOpCode.WREQ.id.U)
    c.ctrl_io.rocc_cmd.cmd_inst.funct.poke(interface)
    c.ctrl_io.rocc_cmd.cmd_rs1.poke(index)
    c.ctrl_io.rocc_cmd.cmd_rs2.poke(0.U)
    c.ctrl_io.rocc_cmd.cmd_valid.poke(true.B)
    ...
    c.ctrl_io.rocc_cmd.cmd_inst.opcode.poke(SyGSOpCode.WMEM.id.U)
    c.ctrl_io.rocc_cmd.cmd_inst.funct.poke(0.U)
    c.ctrl_io.rocc_cmd.cmd_rs1.poke(dataIn1)
    c.ctrl_io.rocc_cmd.cmd_rs2.poke(dataIn2)
    c.ctrl_io.rocc_cmd.cmd_valid.poke(true.B)
  ...
  def writeMemChunkFloat(data: ListBuffer[java.lang.Float], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    writeMemChunkUInt(data.map(floatToUInt), interface, roundRobinAddresses, c)
  }

  def writeMemChunkDouble(data: ListBuffer[Double], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    writeMemChunkUInt(data.map(doubleToUInt), interface, roundRobinAddresses, c)
  }

  def writeMemChunkInt(data: ListBuffer[Int], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    writeMemChunkUInt(data.map(intToUInt), interface, roundRobinAddresses, c)
  }

  def writeMemChunkLong(data: ListBuffer[Long], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    writeMemChunkUInt(data.map(longToUInt), interface, roundRobinAddresses, c)
  }

  def writeMemChunkUInt(data: ListBuffer[UInt], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {
  ...
  def checkMemFloat(data: ListBuffer[java.lang.Float], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    checkMemUInt(data.map(floatToUInt), interface, roundRobinAddresses, c)
  }

  def checkMemDouble(data: ListBuffer[Double], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    checkMemUInt(data.map(doubleToUInt), interface, roundRobinAddresses, c)
  }

  def checkMemInt(data: ListBuffer[Int], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    checkMemUInt(data.map(intToUInt), interface, roundRobinAddresses, c)
  }

  def checkMemLong(data: ListBuffer[Long], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {

    checkMemUInt(data.map(longToUInt), interface, roundRobinAddresses, c)
  }

  def checkMemUInt(correctData: ListBuffer[UInt], interface: Int, roundRobinAddresses: Boolean, c: SyGSTemplate): Unit = {
    ...
    for (i <- correctData.indices) {
      ...
      if(i < correctData.size) {
        ...
        c.ctrl_io.rocc_cmd.cmd_inst.opcode.poke(SyGSOpCode.RMEM.id.U)
        c.ctrl_io.rocc_cmd.cmd_inst.funct.poke(interface.U)
        c.ctrl_io.rocc_cmd.cmd_rs1.poke(varIndex.U)
        c.ctrl_io.rocc_cmd.cmd_rs2.poke(0.U)
        c.ctrl_io.rocc_cmd.cmd_valid.poke(true.B)
        ...
        var res = c.ctrl_io.rocc_cmd.resp_data.peekInt() << 32
        //println(c.ctrl_io.rocc_cmd.resp_data.peekInt())
        println(res)

        c.ctrl_io.rocc_cmd.cmd_inst.opcode.poke(SyGSOpCode.RMEM.id.U)
        c.ctrl_io.rocc_cmd.cmd_inst.funct.poke(interface.U)
        c.ctrl_io.rocc_cmd.cmd_rs1.poke(varIndex.U)
        c.ctrl_io.rocc_cmd.cmd_rs2.poke(1.U)
        c.ctrl_io.rocc_cmd.cmd_valid.poke(true.B)
        ...
        res += c.ctrl_io.rocc_cmd.resp_data.peekInt()
